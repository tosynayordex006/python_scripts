...
{	
	...
	PARFILE_LOC="/home/Cloudserver/scripts/favour_pd"
	counter=1
	...
	sqlplus -s clouddb_temp/Guitar0679 <<EOF
	select * from global_name;
	SELECT sum(bytes)/1024/1024 AS "Size in MB" FROM dba_data_files;
	show user;
	spool /home/Cloudserver/scripts/favour_pd/database_user.log
...
EOF
	...
	do
		...
		then
			...
			echo "dumpfile=expdp_${SCHEMA}_${TS}.dmp">> ${PARFILE_LOC}/expdp_${SCHEMA}_${TS}.par
			echo "logfile=expdp_${SCHEMA}_${TS}.log">>${PARFILE_LOC}/expdp_${SCHEMA}_${TS}.par
	...
	grep "successfully" ${BK_LOC}/expdp_${SCHEMA}_${TS}.log>>${LOG_LOCATION}/${LOG_FILE}
	grep "ORA-" ${BK_LOC}/expdp_${SCHEMA}_${TS}.log>>${LOG_LOCATION}/${LOG_FILE}
